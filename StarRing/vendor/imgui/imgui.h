...
// Forward declarations（这里的数据结构是ImGui所有用到的结构）
...
namespace ImGui
{
    ...
    //      Begin()的作用是创建一个新的窗口，然后可以给这个窗口配置渲染数据，并把这个窗口压入渲染堆栈。
    //      End()的作用是把当前的窗口pop出栈，并且结束对于这个窗口的渲染数据配置过程
    // 
    // - Passing 'bool* p_open != NULL' shows a window-closing widget in the upper-right corner of the window,
    //   which clicking will set the boolean to false when clicked.
    // - You may append multiple times to the same window during the same frame by calling Begin()/End() pairs multiple times.
    //   
    //      'flags' or 'p_open'只会对于调用了begin()和end()的窗口生效，不对内部child起作用
    //   Some information such as 'flags' or 'p_open' will only be considered by the first call to Begin().
    //   
    //      当窗口被阻塞或者被遮挡剔除的时候，会返回false
    ...
    //    BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding BeginXXX function
    // 
    //      Begin() | End()和 BeginChild() | EndChild()是特例：不管begin返回值是啥，都必须end。
    //      除了这俩特例之外，别的end都是在begin返回true的时候才调用
    //      
    //    returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
    ...
    //获取当前window的视口（一个window一个视口，视口就是window内部能渲染的部分）
    IMGUI_API ImGuiViewport*GetWindowViewport();                        // get viewport currently associated to the current window.

    // Window manipulation
    // - Prefer using SetNextXXX functions (before Begin) rather that SetXXX functions (after Begin).

    //这里的API可以提前设置下一个window的渲染数据
    ...
    // Content region
    // - Retrieve（检索） available space from a given point. GetContentRegionAvail() is frequently useful.
    ...
    IMGUI_API void          PushStyleColor(ImGuiCol idx, ImU32 col);                        // modify（修改） a style color. always use this if you modify the style after NewFrame().
    ...
    IMGUI_API void          PushButtonRepeat(bool repeat);  //持续按下                                // in 'repeat' mode, Button*() functions return repeated true in a typematic manner (using io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you can call IsItemActive() after any Button() to tell if the button is held in the current frame.
...
struct ImGuiIO
{
    ...
    //后端平台名称+后端渲染器名称+后端平台数据+后端渲染器数据+后端语言数据
    const char* BackendPlatformName;            // = NULL
    const char* BackendRendererName;            // = NULL

    //这个BackendPlatformUserData是glfw的后端数据包
    void*       BackendPlatformUserData;        // = NULL           // User data for platform backend
    
    //这个BackendRendererUserData是OpenGL的后端数据包
    ...
    IMGUI_API void  AddFocusEvent(bool focused);                // Notifies(通知) Dear ImGui when hosting platform windows lose or gain input focus
...
// 
// 每个window都有自己的ImDrawList，
// Each dear imgui window contains its own ImDrawList. You can use ImGui::GetWindowDrawList() to
// access the current window draw list and draw custom primitives.
// You can interleave（插进） normal ImGui:: calls and adding primitives to the current draw list.
// 
// 单视口渲染模式：左上角是渲染的视口原点
// In single viewport mode, top-left is == GetMainViewport()->Pos (generally 0,0), bottom-right is == GetMainViewport()->Pos+Size (generally io.DisplaySize).
// You are totally free to apply whatever transformation matrix to want to the data (depending on the use of the transformation you may want to apply it to ClipRect as well!)
// Important: Primitives are always added to the list and not culled (culling is done at higher-level by ImGui:: functions), if you use this API a lot consider coarse(粗糙的) culling your drawn objects.
struct ImDrawList
{
    ...
    //分流器
    ImDrawListSplitter      _Splitter;          // [Internal] for channels api (note: prefer using your own persistent（连续的） instance of ImDrawListSplitter!)
    float                   _FringeScale;       // [Internal] anti-alias fringe（边缘） is scaled by this value, this helps to keep things sharp while zooming（快速移动） at vertex buffer content

    // If you want to create ImDrawList instances, pass them ImGui::GetDrawListSharedData() or create and use your own ImDrawListSharedData (so you can use ImDrawList without ImGui)
    
    //ImDrawListSharedData结构可以自己定义
    ...
    //p_min是视口左上角，uv_min是p_min对应的纹理坐标。p_max是视口右下角，uv_max是p_max对应的纹理坐标。
    // Image primitives
    ...
    inline    void  PathFillConvex(ImU32 col)                                   { AddConvexPolyFilled(_Path.Data, _Path.Size, col); _Path.Size = 0; }  // Note: Anti-aliased filling requires points to be in clockwise（顺时针） order.
    ...
    //AddDrawCmd()会强制添加一条新的DrawCall，自定义实现blend，或者特殊的渲染操作
    ...
    // Advanced: Channels
    // 用Channels来把渲染分成好几拨，可以自己设置渲染顺序
    // - Use to split render into layers. By switching channels to can render out-of-order (e.g. submit FG（前景） primitives before BG（背景） primitives)
    // - Use to minimize draw calls (e.g. if going back-and-forth（来回） between multiple clipping rectangles, prefer to append into separate channels then merge at the end)
    // - FIXME-OBSOLETE: This API shouldn't have been in ImDrawList in the first place（首先）!
    ...
    // - All primitives needs to be reserved via PrimReserve() beforehand（事先）.
...
//ImDrawData保存着渲染一帧所需要的所有渲染数据,维护所有的ImDrawList
struct ImDrawData
{
...
struct ImFontGlyph
{
    unsigned int    Colored : 1;        // Flag to indicate glyph（象形符号） is colored and should generally ignore tinting (make it usable with no shift on little-endian as this is used in loops)
