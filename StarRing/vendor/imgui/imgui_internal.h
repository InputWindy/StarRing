...
// 
// 所有ImDrawList都共享的渲染数据
// You may want to create your own instance of this if you want to use ImDrawList completely without ImGui. In that case, watch out for future changes to this structure.
struct IMGUI_API ImDrawListSharedData
{
    //UV坐标
    ImVec2          TexUvWhitePixel;            // UV of white pixel in the atlas（图集）
    ImFont*         Font;                       // Current/default font (optional, for simplified AddText overload)
    float           FontSize;                   // Current/default font size (optional, for simplified AddText overload)
    float           CurveTessellationTol;//曲线细分       // Tessellation tolerance（曲线细分尺度） when using PathBezierCurveTo()
    float           CircleSegmentMaxError;//圆分割     // Number of circle segments to use per pixel of radius for AddCircle() etc
    ...
    const ImVec4*   TexUvLines;                 // UV of anti-aliased lines in the atlas（图集）
...
struct ImDrawDataBuilder
{
    ...
    IMGUI_API void FlattenIntoSingleLayer();//变成一个layer
};
...
// Stacked color modifier, backup of modified data so we can restore it（backup：备份）
...
struct IMGUI_API ImGuiDockNode
{
    ...
    ImRect                  Rect() const            { return ImRect(Pos.x, Pos.y, Pos.x + Size.x, Pos.y + Size.y); }//左下角跟右上角
...
namespace ImGui
{
    ...
    //ImGui接口函数只能在newFrame和Render之前调用
    ...
    IMGUI_API bool          IsWindowChildOf(ImGuiWindow* window, ImGuiWindow* potential_parent, bool dock_hierarchy);
